						...
						<div class="col-sm-6">
							<div class="form-group">
								...
								<select name="billable_type"
                                        v-model="billable_type"
                                        required
                                        class="form-control">
									<option selected>Please Select</option>
									<option value="App\Client">Company</option>
									<option value="App\Project">Project</option>
								</select>
							</div>
						</div>
						<div class="col-sm-6">
							<div class="form-group">
								<label class="form-label"
                                       v-text="billable"></label>
								<select v-model="billable_id"
                                        name="billable_id"
                                        class="form-control"
                                        :disabled="isDisabled"
                                >
									<option v-for="option in options"
                                            :value="option.id"
                                            :key="option.id">{{option.name || option.title}}</option>
									</select>
					</div>
						</div>
					</div>
...
<script>
    ...
    export default {
      ...
      computed  : {
        billable() {
          this.isDisabled = !(this.billable_type === "App\\Client" || this.billable_type === "App\\Project")
          this.billable_id = null
          this.options = []
          if (this.billable_type === null || this.billable_type === "Please Select" || this.billable_type === "App\\Client") {
            if (this.billable_type === "App\\Client") {
              axios.get('create/type?type=' + this.billable_type)
